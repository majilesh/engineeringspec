...
describe("portable conformance fixtures",()=>{
  for(const fixture of manifest.fixtures) it(fixture.file,async()=>{const result=await validateFile(`conformance/${fixture.file}`,{strictExternal:true});expect(result.valid).toBe(fixture.valid);const codes=result.diagnostics.map(d=>d.code);for(const code of fixture.codes) expect(codes).toContain(code);});
