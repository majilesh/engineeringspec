...
describe("parser and normalized model",()=>{
  ...
  it("appends a hint when a diagnostic carries one",()=>{expect(formatDiagnostics([{code:"C",severity:"error",message:"broken",hint:"try this"}])).toBe("error C broken (hint: try this)");});
