...
Prefer the repository-local `engineeringspec` binary when the package is installed. The fallback commands below pin the prepared RC16 CLI identity; they become externally installable only after the separate publication ceremony. Do not replace the exact version with a mutable distribution tag in an enforcing workflow.
...
```sh
npx --yes @engineeringspec/cli@0.1.0-rc.16 propose --id ES-change --title "Change title" --owner engineering \
...
```sh
npx --yes @engineeringspec/cli@0.1.0-rc.16 explain <selected-spec> --path <path> --change-kind modified --base origin/main
```
