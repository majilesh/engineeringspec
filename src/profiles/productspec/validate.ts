...
export interface ProfileOptions { strictExternal?:boolean; resolveProfiles?:boolean; repositoryRoot?:string }
...
export async function validateProductSpecProfile(spec:EngineeringSpec,specFile:string,options:ProfileOptions={}):Promise<Diagnostic[]> {
  ...
  for(const source of spec.sourceRefs.filter(source=>source.type==="productspec")) {
    ...
    try {
      const resolved=await resolveProductSpec(await resolveRepositoryPath(options.repositoryRoot,source.path));
