...
export async function nextAction(options: { base?: string; cwd?: string } = {}): Promise<NextReport> {
  ...
  const approvedIds = status.routing.candidates.filter((item) => item.eligible && item.authorityKind !== "standing" && !item.spent).map((item) => item.specId);
...
export function nextTicket(report: NextReport): NextTicket {
  ...
  return {
    ...
    command,
    approvedIds: [...new Set(routing.candidates.filter((item) => item.eligible && item.authorityKind !== "standing" && !item.spent).map((item) => item.specId))].sort(compareCodePoints),
